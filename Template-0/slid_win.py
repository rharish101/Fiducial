...
def sliding_windows(image, model_name, model_input_shape, label,
    ...
    for mini_image in pyramid(image, resize_factor, min_size):
        for micro_image_box in windows(mini_image, window_size, stride_size):
            ...
            prediction = model.predict(np.resize(micro_image,
                model_input_shape), batch_size=1)
            if prediction[0] == label:
                images.append(np.array((micro_image_box, micro_image)))
